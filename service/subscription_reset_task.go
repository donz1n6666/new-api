...
func runSubscriptionQuotaResetOnce() {
	...
	for {
		n, err := model.ExpireStalePendingSubscriptionOrders(subscriptionResetBatchSize)
		if err != nil {
			logger.LogWarn(ctx, fmt.Sprintf("subscription pending expire task failed: %v", err))
			return
		}
		if n == 0 {
			break
	...
	if common.DebugEnabled && (totalReset > 0 || totalExpired > 0 || totalPendingExpired > 0) {
		logger.LogDebug(ctx, "subscription maintenance: reset_count=%d, expired_count=%d, pending_expired_count=%d", totalReset, totalExpired, totalPendingExpired)
	}
}
