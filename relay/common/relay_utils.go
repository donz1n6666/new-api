...
func GetFullRequestURL(baseURL string, requestURL string, channelType int) string {
	// 1. 如果 baseURL 已经包含了常见的 API 路径后缀（用户填写了完整URL），直接使用 baseURL
	// 支持的路径: /chat/completions, /v1/chat/completions, /messages, /v1/messages 等
	commonPaths := []string{
		"/chat/completions",
		"/messages",
		"/models",
	...
	// 使用正则匹配 /v 开头后面跟数字/字母的版本号
	matched, _ := regexp.MatchString(`/v[a-zA-Z0-9]+$`, baseURL)
	if matched {
		// requestURL 通常是 /v1/xxx，去掉 /v1 前缀，只保留 /xxx 部分
