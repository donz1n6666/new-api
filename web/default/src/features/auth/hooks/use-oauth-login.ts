...
export function useOAuthLogin(status: SystemStatus | null) {
  ...
  const handleMisskeyLogin = async () => {
    setIsLoading(true)
    try {
      await resetSession()
      // State and PKCE verifier are generated by the backend authorize
      // endpoint; only the affiliate code needs to be forwarded.
      const affCode = localStorage.getItem('aff')
      const url =
        affCode && affCode.length > 0
          ? `/api/oauth/misskey/authorize?aff=${encodeURIComponent(affCode)}`
          : '/api/oauth/misskey/authorize'
      window.open(url, '_self')
    } catch (_error) {
      toast.error(t('Failed to start Misskey login'))
      setIsLoading(false)
    }
  }
